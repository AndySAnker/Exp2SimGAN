...
from . import ac_networks3D as networks
from .patchnce import PatchNCELoss
import util.util as util
from util.image_pool import ImagePool


class ACDCL3DModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        print ("\n >>> WARNING: You are using the 3D network, which does not have all functionalities implemented. Therefore, use default options or look into source-code. <<< \n")
        ...
        self.netG_A = networks.define_G_3D(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias,
                                        opt.no_antialias_up, self.gpu_ids, opt)
        self.netG_B = networks.define_G_3D(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG,
                                        not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias,
                                        opt.no_antialias_up, self.gpu_ids, opt)
        self.netF1 = networks.define_F_3D(opt.input_nc, opt.netF, opt.normG,
                                       not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids,
                                       opt)
        self.netF2 = networks.define_F_3D(opt.input_nc, opt.netF, opt.normG,
                                       not opt.no_dropout, opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids,
                                       opt)

        if self.isTrain:
            self.netD_A = networks.define_D_3D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias,
                                            self.gpu_ids, opt)
            self.netD_B = networks.define_D_3D(opt.output_nc, opt.ndf, opt.netD,
                                            opt.n_layers_D, opt.normD, opt.init_type, opt.init_gain, opt.no_antialias,
                                            self.gpu_ids, opt)
            ...
            self.criterionClass = torch.nn.MSELoss().to(self.device)
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        """
        # Real
        pred_real_ph = netD(real)
        pred_real = pred_real_ph[:,0,:,:].unsqueeze(0)
        pred_real_class = pred_real_ph[:,1,:,:].unsqueeze(0)
        loss_D_real = self.criterionGAN(pred_real, True)
        # Fake
        pred_fake_ph = netD(fake.detach())
        pred_fake = pred_fake_ph[:,0,:,:].unsqueeze(0)
        pred_fake_class = pred_fake_ph[:,1,:,:].unsqueeze(0)
        loss_D_fake = self.criterionGAN(pred_fake, False)
        # Calculate class loss
        real_class = torch.zeros(pred_real_class.shape).to(self.device)
        fake_class = torch.ones(pred_fake_class.shape).to(self.device)
        real_class_loss = self.criterionClass(pred_real_class, real_class)
        fake_class_loss = self.criterionClass(pred_fake_class, fake_class)
        # Combined loss and calculate gradients
        loss_D = (loss_D_real + loss_D_fake + real_class_loss + fake_class_loss) * 0.25
        loss_D.backward()
        return loss_D
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_GAN > 0.0:
            ...
            pred_fakeB = self.netD_A(fakeB)
            pred_fakeB = pred_fakeB[:,0,:,:].unsqueeze(0)
            pred_fakeA = self.netD_B(fakeA)
            pred_fakeA = pred_fakeA[:,0,:,:].unsqueeze(0)
            self.loss_G_A = self.criterionGAN(pred_fakeB, True).mean() * self.opt.lambda_GAN
            self.loss_G_B = self.criterionGAN(pred_fakeA, True).mean() * self.opt.lambda_GAN
        else:
            self.loss_G_A = 0.0
