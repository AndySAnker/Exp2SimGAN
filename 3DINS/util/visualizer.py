...
def save_images(webpage, visuals, image_path, aspect_ratio=1.0, width=256):
    ...
    for label, im_data in visuals.items():
        ...
        if im_data.ndim - 2 == 1:
            util.save_image1D(im_data.cpu().detach().numpy(), save_path)
        if im_data.ndim - 2 == 2:
            im = util.tensor2im(im_data)
            util.save_image(im, save_path, aspect_ratio=aspect_ratio)
        if im_data.ndim - 2 == 3:
            util.save_image3D(im_data.cpu().detach().numpy(), save_path)
    ...
    def display_current_results(self, visuals, epoch, save_result):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to an HTML file if they haven't been saved.
            ...
            for label, image in visuals.items():
                ...
                if image.ndim - 2 == 1:
                    util.save_image1D(image.cpu().detach().numpy(), img_path)
                if image.ndim - 2 == 2:
                    image_numpy = util.tensor2im(image)
                    util.save_image(image_numpy, img_path)
                if image.ndim - 2 == 3:
                    util.save_image3D(image.cpu().detach().numpy(), img_path)
            ...
            for n in range(epoch, 0, -1):
                ...
                for label, image_numpy in visuals.items():
                    if image_numpy.ndim - 2 == 3:
                        img = image_numpy[0,0,:,:,:]
                        image = img[:,:,img.shape[-1]//2].unsqueeze(0).unsqueeze(0)
                    image_numpy = util.tensor2im(image)
